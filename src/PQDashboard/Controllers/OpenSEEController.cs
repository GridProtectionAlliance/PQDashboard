...
namespace OpenSEE.Controller
{
    ...
    public class OpenSEEController : ApiController
    {
        ...
        public const string TimeCorrelatedSagsSQL =
            ...
            "    Event.StartTime, Meter.Name AS MeterName, " +
            "    MeterLine.LineName " +
            "FROM " +
            "    Event JOIN " +
            ...
            using (AdoDataConnection connection = new AdoDataConnection("systemSettings"))
            {
                    ...
                    foreach (string key in temp.Keys)
                    {
                        if (temp[key].DataPoints.Count() > 0)
                        {
                            if (dict.ContainsKey(key))
                                dict[key].DataPoints.AddRange(temp[key].DataPoints);
                            else
                ...
                if (dataType == "Statistics")
                {
                    foreach (string key in dict.Keys)
                    {
                        ...
                        series.DataPoints.Sort((a, b) => a[0].CompareTo(b[0]));
                ...
                foreach (string key in dict.Keys)
                {
                    ...
                    List<double[]> dataPoints = dict[key].DataPoints;
                    dataPoints.Sort((a, b) => a[0].CompareTo(b[0]));
                    series.DataPoints = Downsample(dataPoints, pixels, new Range<DateTime>(startTime, endTime));
